...
Examples:
  ...
  forensic-surgeon --generate-reports html,json --output ./reports/
    ...
    # Tracking History Commands (NEW!)
    parser.add_argument('--scan-history', action='store_true',
                       help='Scan browser history to reconstruct past tracking')
    
    parser.add_argument('--start', action='store_true',
                       help='Start 24/7 background monitoring (silent)')
    
    parser.add_argument('--stop-monitor', action='store_true',
                       help='Stop background monitoring service')
    
    parser.add_argument('--query-tracking', nargs=2, metavar=('START_DATE', 'END_DATE'),
                       help='Query tracking database by date range (YYYY-MM-DD)')
    
    parser.add_argument('--reality-check', action='store_true',
                       help='Launch Reality Check dashboard (THE ONE dashboard)')
    ...
    try:
        ...
        if args.start:
            ...
            print("💡 Use --reality-check to view your data")
            print("💡 Use --stop-monitor to stop monitoring")
            return 0
        ...
        if args.query_tracking:
            ...
            db = TrackingDatabase()
            start_date, end_date = args.query_tracking
            results = db.query_events_by_date(start_date, end_date)
            print(f"\n📊 Found {len(results)} tracking events")
            for event in results[:20]:  # Show first 20
                print(f"  {event[0]} - {event[1]} - {event[2]}")
            if len(results) > 20:
                print(f"  ... and {len(results) - 20} more")
            print(f"\n💡 Use --tracking-dashboard for full analysis")
            return 0
        
        if args.reality_check:
            import subprocess
            subprocess.run([sys.executable, "-m", "streamlit", "run", "reality_check_dashboard.py"])
            return 0
        ...
        if not any([args.full_scan, args.quick_scan, args.target, args.list_services,
                   ...
                   args.validate_db, args.version, args.info, args.reality_check]):
            return cli.interactive_mode()

...
class AlertSystem:
    @staticmethod
    def trigger_critical(message, destination):
        print(f"CRITICAL ALERT: {message} to {destination}")
...
class PacketDataAnalyzer(BaseScanner):
    ...
    def __init__(self, config: Optional[ForensicConfig] = None):
        ...
        self.user_config = config.browser_profiles if config else None
    ...
    def scan(self) -> Generator[EvidenceItem, None, None]:
        ...
        # If monitoring is active, yield from the queue
        if self.is_monitoring:
            while not self.packet_queue.empty():
                try:
                    packet_data = self.packet_queue.get_nowait()
                    yield EvidenceItem(
                        id=f"packet_{packet_data.source_ip}_{packet_data.dest_ip}_{int(time.time())}",
                        source="packet_analyzer",
                        type="network_packet",
                        content=f"Packet: {packet_data.source_ip} -> {packet_data.dest_ip} ({packet_data.protocol}, {packet_data.size} bytes) [{packet_data.entity_name}]",
        ...
        # Only do snapshot analysis if NOT monitoring
    ...
    def start_monitoring(self):
        ...
        print("Continuous packet monitoring started.")
    ...
    def _monitor_loop(self):
        ...
        if not SCAPY_AVAILABLE:
            print("Scapy not available, cannot monitor.")
            return
        ...
        def packet_callback(packet):
            ...
            try:
                # Analyze packet and put in queue
                analysis = self._analyze_packet_data(packet)
                if analysis:
        ...
        try:
            # Sniff continuously
            scapy.sniff(prn=packet_callback, store=0, stop_filter=lambda p: not self.is_monitoring)
        except Exception as e:
            print(f"Monitoring error: {e}")

    def start_leak_monitor(self):
        """Background thread that sniffs for your Name/Email/Phone in cleartext."""
        # Deprecated in favor of start_monitoring, but kept for compatibility
        ...
        try:
            ...
            for conn in connections:
                if conn.status == 'ESTABLISHED':
                    ...
                    # Get Entity Name
                    domain = self._ip_to_domain(conn.raddr.ip if conn.raddr else 'unknown')
                    entity_name = self.entity_mapper.get_entity(domain)
    ...
    def _analyze_packet_data(self, packet) -> Optional[DataPacket]:
        """Analyze individual packet and return DataPacket object."""
        try:
            if packet.haslayer(scapy.IP):
                ...
                # Get Entity Name
                domain = self._ip_to_domain(dst_ip)
                entity_name = self.entity_mapper.get_entity(domain)
    ...
    def _analyze_packet(self, packet) -> Optional[EvidenceItem]:
        """Analyze individual packet for content and destinations (Legacy wrapper)."""
        packet_data = self._analyze_packet_data(packet)
        if packet_data:

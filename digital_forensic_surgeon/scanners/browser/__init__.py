...
class BrowserScanner:
    ...
    def _scan_profile(self, profile: BrowserProfile) -> Generator[EvidenceItem, None, None]:
        ...
        yield from self._extract_history_timeline(profile)
    ...
    def _extract_history_timeline(self, profile: BrowserProfile) -> Generator[EvidenceItem, None, None]:
        """
        UPGRADE: Aggregates history to show WHO is tracking you and WHEN.
        """
        try:
            history_path = profile.profile_path / "History"
            ...
            try:
                with sqlite3.connect(temp_db) as conn:
                    ...
                    # Query to group visits by Domain and Date (Day)
                    query = """
                        SELECT 
                            ...
                            visit_count
                        FROM urls 
                        WHERE visit_date > datetime('now', '-30 days') -- Focus on recent "Health"
                    ...
                    if rows:
                        for row in rows:
                            try:
                                visit_date, url, title, visit_count = row
                                yield EvidenceItem(
                                    type="browser_history",
                                    source="browser",
                                    path=url,
                                    content=f"Visited {url} on {visit_date}",
                                    metadata={
                                        "browser": profile.browser,
                                    ...
                                    is_sensitive=True,
                                    confidence=0.8,
                                    timestamp=datetime.fromisoformat(visit_date)
                                )
                            except (ValueError, IndexError):
                                continue
            finally:
            ...
            yield EvidenceItem(
                type="browser_history_error",
                source="browser",
                path=str(profile.profile_path),
                content=f"Failed to extract history timeline: {e}",
                is_sensitive=True,
                confidence=0.1
            )
